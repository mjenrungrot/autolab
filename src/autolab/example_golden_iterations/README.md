# Golden Iteration Example

This folder contains a complete example iteration with stage artifacts that match current scaffold schemas and verifier expectations.

## Layout

- `.autolab/state.json`
- `.autolab/backlog.yaml`
- `.autolab/plan_check_result.json`
- `.autolab/plan_contract.json`
- `.autolab/plan_graph.json`
- `experiments/plan/iter_golden/hypothesis.md`
- `experiments/plan/iter_golden/design.yaml`
- `experiments/plan/iter_golden/implementation_plan.md`
- `experiments/plan/iter_golden/plan_contract.json`
- `experiments/plan/iter_golden/implementation_review.md`
- `experiments/plan/iter_golden/review_result.json`
- `experiments/plan/iter_golden/launch/run_local.sh`
- `experiments/plan/iter_golden/runs/20260201T120000Z_demo/run_manifest.json`
- `experiments/plan/iter_golden/runs/20260201T120000Z_demo/metrics.json`
- `experiments/plan/iter_golden/analysis/summary.md`
- `experiments/plan/iter_golden/docs_update.md`
- `experiments/plan/iter_golden/decision_result.json`
- `paper/results.md`

## Usage

Copy these artifacts into a test repository to compare against your current iteration outputs and verifier failures.
