## VERIFICATION RITUAL (6 steps -- run before marking stage complete)
...
5. **Dual-memory audit**: Confirm `docs/todo.md` and `{{iteration_path}}/documentation.md` are synchronized and non-contradictory for actionable work and completion status.
6. **Scope audit**: Confirm every file you created or edited is inside the `allowed_edit_dirs` from your runtime context. If you touched anything outside, revert it.

Do not declare the stage complete until all six steps pass. If a verifier fails, fix the artifact and rerun from step 2.
