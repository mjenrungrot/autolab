...
python -m pip install git+https://github.com/mjenrungrot/autolab.git@v1.2.50
```
...
**Progress, handoff, and resume.** `autolab progress` refreshes and summarizes takeover state, including critical-path duration/basis, per-wave timing windows, retry reasons, blocked/deferred/skipped tasks, file conflicts, per-task evidence summaries, pending implementation plan approvals, active campaign context, and Oracle auto status when present. `autolab handoff` writes both handoff artifacts: machine JSON (`.autolab/handoff.json`) and human Markdown (`<scope-root>/handoff.md`). The machine snapshot now also carries a nested `continuation_packet`, which is the richer scope-root continuation envelope used for downstream takeover/export flows. When a campaign is active, those pointers also surface generated `results.md` and `results.tsv` ledgers under the same scope root. `autolab oracle` materializes that packet into an on-demand scope-root `oracle.md` expert-review handoff: a neutral, self-contained document with repo-relative paths and focused inlined excerpts from the minimum files needed for outside review, and stamps `last_oracle_at` in `.autolab/campaign.json` when a campaign is active. `autolab oracle roundtrip --auto` reuses that handoff, runs one browser-only Oracle session, captures `.autolab/oracle_last_response.md`, records `.autolab/oracle_state.json`, and applies the reply advisory-only when ingestion succeeds. `autolab oracle apply <reply.md>` also accepts `--notes <file>` or `--stdin` and accepts either structured `ReviewerVerdict:` replies or free-form expert feedback. Structured replies are ingested directly; otherwise Autolab can fall back to a local ingestion LLM configured through `oracle_apply.ingestion_mode`, `oracle_apply.llm_command`, and `oracle_apply.llm_timeout_seconds` in `.autolab/verifier_policy.yaml`, or the `AUTOLAB_ORACLE_APPLY_AGENT_COMMAND` environment override. Writes remain advisory-only and are still filtered through the `oracle_apply` allowlist before they touch discuss/research sidecars, TODO hints, campaign oracle feedback, or plan-approval notes. `autolab resume` previews the recommended next command and can execute it with `--apply` when the safe-resume point is ready. Handoff artifacts are auto-refreshed on verification updates, each run/loop iteration, campaign-status changes, and manual stage-steering exits (for example `review`, `approve-plan`, `skip`, `focus`, and `experiment move`); `oracle.md` is emitted only when requested.
...
Cockpit handoff surfaces remain backed by `.autolab/handoff.json`; generate the expanded scope-root `oracle.md` handoff separately with `autolab oracle` when you need the self-contained external-review packet.
